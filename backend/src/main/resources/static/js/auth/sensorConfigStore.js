...
const DEFAULTS = {
  ingest: { enabled: true, demoPushEnabled: false, demoPushIntervalSec: 30 },
  ...
  mqtt: {
    enabled: true,
    host: '121.196.215.170',
    port: 1883,
    username: 'admin',
    password: '',
    clientId: 'mine-onemap-api',
    topic: '',
    topics: ['$iot/an20260811/gnss', '$iot/an20260811/YK'],
    keepalive: 60,
...
export function toGatewayConfigPayload(adminCfg) {
  ...
  return {
    ...
    mqtt: {
      ...
      clientId: c.mqtt.clientId || 'mine-onemap-api',
      topic: c.mqtt.topic || '',
      topics: Array.isArray(c.mqtt.topics)
        ? c.mqtt.topics.map((t) => String(t || '').trim()).filter(Boolean)
        : [],
